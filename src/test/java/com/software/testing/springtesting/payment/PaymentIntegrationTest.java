...
import com.software.testing.springtesting.customer.CustomerRegistrationRequest;
...
class PaymentIntegrationTest {
    ...
    void itShouldCreatePaymentSuccessfully() throws Exception {
        ...
        // And payment is saved in db
        MvcResult getPaymentResult = mockMvc.perform(get("/api/v1/payment/{paymentId}", paymentId))
                .andReturn();

        Payment actualPayment = mvcResultToObject(getPaymentResult, Payment.class);
        Assertions.assertEquals(payment, actualPayment);
    }
    ...
    private static <T> T mvcResultToObject(MvcResult result, Class<T> valueType) {
        try {
            String contentAsString = result.getResponse().getContentAsString();
            return new ObjectMapper().readValue(contentAsString, valueType);
        } catch (JsonProcessingException | UnsupportedEncodingException e) {
            Assertions.fail(String.format("Failed to convert response of type [%s]", valueType));
            return null;
        }
    }
}
